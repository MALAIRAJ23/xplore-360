...
className="relative w-full px-5 py-10 md:py-16 bg-page-bg flex flex-col items-center overflow-hidden [perspective:1600px]"    >
      ...
      <motion.div
        ...
        className="relative z-10 w-[85%] sm:w-full max-w-[600px] lg:max-w-[750px] flex flex-col items-center aspect-square mx-auto [transform-style:preserve-3d]"
      ...
      >
        ...
        {nodePositions.map((n, idx) => {
          ...
          return (
            <motion.div
              key={n.day}
              className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-4 cursor-pointer z-20 group bg-white/95 backdrop-blur-md border border-slate-200/80 shadow-[0_8px_24px_-8px_rgba(15,23,42,0.12)] p-2 sm:py-4 sm:px-6 sm:pl-4 rounded-[14px] sm:rounded-full [transform-style:preserve-3d]"
            ...
            >
              {/* Icon Circle */}
              <div className="shrink-0 w-7 h-7 sm:w-12 sm:h-12 rounded-full bg-[#635BFF]/10 text-[#635BFF] flex items-center justify-center transition-colors group-hover:bg-[#635BFF] group-hover:text-white">
                <Icon className="w-4 h-4 sm:w-6 sm:h-6" strokeWidth={2.5} />
              </div>

              {/* Text Info */}
              <div className="flex flex-col items-center sm:items-start text-center sm:text-left gap-0.5 px-1 sm:pr-2">
                <span className="text-[9px] sm:text-[13px] font-extrabold tracking-widest text-[#635BFF] uppercase">
                  {n.day}
                </span>
                <span className="text-[10px] sm:text-[17px] font-bold text-slate-900 leading-[1.1] sm:leading-tight whitespace-normal sm:whitespace-nowrap max-w-[70px] sm:max-w-none">
                  {n.title}
                </span>
              </div>
            </motion.div>
